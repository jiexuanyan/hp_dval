...
class CLIPVIT(nn.Module):
    ...
    def forward(self, x, norm_pred = True):
        ...
        prompts, prompts_double, temperature, spatial_T = self.prompt_learner()
        ...
        # Default
            ...
            # score2 = logit_scale * dist_feat @ text_features.t()
            # score2 = dist_feat @ text_features_neg.t()
            # prob_spatial = torch.nn.functional.softmax(logit_scale * score2, dim = 0)
            # score2 = torch.sum(tmp_scale * score2 * prob_spatial)
            # score1 = torch.topk(tmp_scale * pred_feat @ text_features_neg.t(),k=self.topk, dim=1)[0].mean(dim=1)
            # score2 = logit_scale * dist_feat @ text_features.t()
            if norm_pred:
