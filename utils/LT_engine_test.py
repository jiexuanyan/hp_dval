...
model_name = 'ema_First_Stage_lr1e-05_dbl_clip_vit_voc-lt'
# model_name = 'zsclip_clip_vit_coco-lt'
    ...
    def validate(self, model, val_loader):
        ...
        for i, (inputData, target, semantic) in enumerate(val_loader):
            ...
            with torch.no_grad():
                with autocast():
                    # regular model
                    model_output = regular_model(inputData.cuda())
                    if isinstance(model_output, (tuple, list)):
                        regular_cls_output = model_output[2] if len(model_output) >= 3 else model_output[0]
                    else:
                        regular_cls_output = model_output
