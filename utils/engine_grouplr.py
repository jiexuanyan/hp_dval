...
class MultiLabelEngine():
    ...
    def learning(self, models, clip_model, train_loader, val_loader, criterion, optimizer, scheduler,model_name):
        ...
        if self.evaluation:
            ...
            for epoch in range(self.start_epoch+1, self.max_epoch):
                # if epoch == 30:# for voc
                if epoch == 19:#for coco and nus-wide
                    assert 1 == 2
                # train step
                models[0].train()
                self.meter_reset()
                ...
                # if epoch >7: #for coco and nus-wide
                if epoch >14: #for voc 2007
                # evaluate step
                    models[0].eval()
                    self.meter_reset()
                    self.validate(models, clip_model,val_loader, criterion)
                    regular_map, ema_map = self.meter_print(model_name)
                    self.save_checkpoint(models, regular_map, ema_map, epoch,model_name)
                    models[0].train()
    ...
    def validate(self, models, clip_model,val_loader, criterion):
        ...
        for i, (inputData, target, semantic) in enumerate(val_loader):
            ...
            # ema model
            with torch.no_grad():
                _, _,ema_cls_output, _,ema_dist_feat = ema_model.module(inputData.cuda())
                # ema_cls_output, _,ema_dist_feat = ema_model.module(inputData.cuda())
                ema_cls_output = ema_cls_output.float()
                ema_overall_loss = criterion[0](ema_cls_output, target)
                # ema_overall_loss = ranking_lossT(ema_cls_output, target)

                # _,ema_tea_dist_feat = clip_model.encode_image(inputData.cuda())
                # ema_dist_loss = F.l1_loss(ema_dist_feat.float(), ema_tea_dist_feat.float())
                # ema_dist_loss = F.smooth_l1_loss(ema_dist_feat.float(), ema_tea_dist_feat.float())
    ...
    def train(self, models, clip_model, train_loader, criterion, optimizer, scheduler, scaler, epoch):
        ...
        for i, (inputData, target, semantic) in enumerate(train_loader):
            ...
            with torch.no_grad():
                _, _,ema_cls_output, _, ema_dist_feat = ema_model.module(inputData.float())
                # ema_cls_output, _, ema_dist_feat = ema_model.module(inputData.float())
                ema_cls_output = ema_cls_output.float()

            ema_cls_loss = criterion[0](ema_cls_output, target)
            # ema_cls_loss = ranking_lossT(ema_cls_output, target)
    ...
    def save_checkpoint(self, models, regular_map, ema_map, epoch, model_name):
        # save current model
        regular_model = models[0]
        ema_model = models[1]
        checkpoint_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'MKT_LT_checkpoint')
        os.makedirs(checkpoint_dir, exist_ok=True)

        # if not os.path.exists('MKT_fs_checkpoint/'):
        #     os.mkdir('MKT_fs_checkpoint/')
        ...
        # if epoch in [15,16,17,18,19]:# for voc
        # if epoch in [8,9,10,11,12]: #for coco and nus-wide
        #     torch.save({
        #         'epoch': epoch,
        #         'score': ema_map,
        #         'model': 'ema',
        #         'state_dict': ema_model.module.state_dict(),
        #     }, os.path.join('/data2/yanjiexuan/checkpoints/RC-Tran/checkpoint/', "_"+cur))    
