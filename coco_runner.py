...
def main_coco():
    ...
    # model_name = 'First_Stage_al{}_lr{}_{}_clip_vit_topk{}_{}_no_prompt'.format(args.alpha,args.lr,args.loss_function,args.topk,args.dataset)
    ...
    # model_name = 'First_Stage_al{}_lr{}_{}_clip_vit_topk{}_{}'.format(args.alpha,args.lr,args.loss_function,args.topk,args.dataset)
    model_name = 'First_Stage_al{}_lr{}_{}_clip_vit_topk{}_{}_coop'.format(args.alpha,args.lr,args.loss_function,args.topk,args.dataset)
    ...
    if args.dataset == 'coco-lt':
        ...
        train_dataset = build_dataset(dataset=args.dataset, split='train', inp_name='./data/coco/coco_glove_300_coco_sequence.pkl')
    ...
    #                        inp_name='data/coco/coco_glove_word2vec_mlgcn.pkl'
    #                        )
        val_dataset = build_dataset(dataset=args.dataset, split='test', inp_name='./data/coco/coco_glove_300_coco_sequence.pkl')
        ...
        train_dataset = build_dataset(dataset=args.dataset, split='train', inp_name='./data/voc/voc_glove_word2vec.pkl')
        val_dataset = build_dataset(dataset=args.dataset, split='test', inp_name='./data/voc/voc_glove_word2vec.pkl')
    ...
    if args.loss_function == 'dbl':
        if args.dataset == 'coco-lt':
            loss_function = ResampleLoss(
                ...
                reweight_func='rebalance',
                focal=dict(focal=True, balance_param=2.0, gamma=2),
                logit_reg=dict(pos_scale=5.0,neg_scale=2.0, init_bias=0.05),
                map_param=dict(alpha=0.1, beta=10.0, gamma=0.2),
                loss_weight=1.0, freq_file=freq_file
            )
        elif args.dataset == 'voc-lt':
            loss_function = ResampleLoss(
                ...
                reweight_func='rebalance',
                focal=dict(focal=True, balance_param=2.0, gamma=2),
                logit_reg=dict(pos_scale=10.0,neg_scale=5.0, init_bias=0.05),
                map_param=dict(alpha=0.1, beta=10.0, gamma=0.3),
                loss_weight=1.0, freq_file=freq_file
            )
    ...
    # parameters = add_weight_decay(models[0].visual_encoder, weight_decay)
    ...
    # print("Turning off gradients in the text encoder and prompt learner")
    # for name, param in models[0].named_parameters():
    #     if "visual_encoder" not in name:
    #         param.requires_grad = False
