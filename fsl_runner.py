...
IDX_ROOT = os.path.join(PROJECT_ROOT, 'idx')
PRETRAINED_ROOT = os.path.join(PROJECT_ROOT, 'pretrained')

parser = argparse.ArgumentParser(description='PyTorch MS_COCO Training')
parser.add_argument('--data', help='path to dataset', default='/data2/yanjiexuan/coco/data')
...
def main_coco():
    ...
    if args.dataset == 'coco':
        ...
        inp_seman = os.path.join(DATA_ROOT, 'coco', 'coco_glove_300_coco_sequence.pkl')
        ...
        args.num_classes = 65
        args.data = os.path.join(DATA_ROOT, 'nuswide')
        inp_seman = os.path.join(DATA_ROOT, 'nuswide', 'nuswide_glove_word2vec.pkl')
        train_dataset = NUSWIDEClassification_fsl(args.data, os.path.join(IDX_ROOT, 'nus', 'classification_Train_base.csv'),
        ...
        val_dataset = NUSWIDEClassification_fsl(args.data, os.path.join(IDX_ROOT, 'nus', 'classification_Test_base.csv'),
                                             word_emb_file=inp_seman,
                                             transform=transforms.Compose([
                                                 transforms.Resize((args.image_size, args.image_size)),
                                                 transforms.ToTensor()
        ...
        args.data = os.path.join(DATA_ROOT, 'voc')
        inp_seman = os.path.join(DATA_ROOT, 'voc', 'voc_glove_word2vec.pkl')
        train_dataset = Voc2007Classification_fsl(args.data, os.path.join(IDX_ROOT, 'voc', 'classification_Train_base.csv'),
                                            word_emb_file = inp_seman,
                                            transform=transforms.Compose([
                                                   transforms.Resize((args.image_size, args.image_size)),
                                                   CutoutPIL(cutout_factor=0.5),
        ...
        val_dataset = Voc2007Classification_fsl(args.data, os.path.join(IDX_ROOT, 'voc', 'classification_Test_base.csv'),
                                           word_emb_file = inp_seman,
                                           transform=transforms.Compose([
                                                   transforms.Resize((args.image_size, args.image_size)),
                                                   transforms.ToTensor(),
