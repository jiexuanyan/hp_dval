...
parser.add_argument('--resume', default=os.path.join(CHECKPOINT_ROOT, 'ema_fsl_First_Stage_al0.5_lr1e-05_clip_vit_topk16_coco_best_83.590_e15.ckpt'), type=str, metavar='PATH',
...
def main():
    ...
    if len(regular_maps)!=10:
        ...
        filename = os.path.join(LOG_ROOT, "uncomplete_a"+str(args['alpha'])+"_"+str(args['shot'])+"shot_"+args['resume'][args['resume'].rfind("/")+1:].replace(".ckpt","_{}_{}.txt".format(feature, mean_)))
    else:
        log_fsl_dir = os.path.join(LOG_ROOT, 'log_fsl')
        os.makedirs(log_fsl_dir, exist_ok=True)
        filename = os.path.join(log_fsl_dir, "a"+str(args['alpha'])+"_"+str(args['shot'])+"shot_"+args['resume'][args['resume'].rfind("/")+1:].replace(".ckpt","_{}_{}.txt".format(feature, mean_)))
